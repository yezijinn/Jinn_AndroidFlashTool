...
> 版本: v3.1（PySide6 石墨暗黑主题 + Frameless 自绘标题栏）
...
**核心原则**：文字对比度 > 信息层级 > 控件状态 > 装饰效果。界面视觉统一由"语义别名"驱动，业务代码不散落任何十六进制色值；所有控件状态（rest/hover/pressed/focus/disabled）在应用级 QSS 内全覆盖。自绘标题栏由主题层统一管理颜色、按钮状态与窗口控制，彻底消除"OS 标题栏颜色与程序背景不一致"问题。
...
```
flash_tool/
├── flash_tool.py          # 主程序：业务逻辑 + 窗口装配
├── aurora_theme.py        # 主题层：Token/调色板/QSS渲染/自绘标题栏
├── assets/
...
| 文件 | 角色 | 关注点 |
|------|------|--------|
| `aurora_theme.py` | **外观唯一权威** | 颜色、字体、间距、状态全部集中于此 |
...
### 3.1 语义化 Token（石墨暗黑调色板，Windows Fluent Dark 风格）
...
TOKENS = {
    'bg_base':       '#111111',   # 主背景
    'bg_elevated':   '#181818',   # 表面层（标题栏/状态栏/分组框）
    'bg_card':       '#202020',   # 卡片层
    ...
    'text_muted':    '#8F8F8F',   # 辅助文字
    'text_disabled': '#666666',   # 禁用文字
    'border':        '#353535',   # 边框
    'accent':        '#4CC2FF',   # 主强调色（低饱和蓝）
    'focus':         '#66CCFF',   # 键盘焦点
    'success':       '#4CC38A',   # 成功
    'warning':       '#E8B04A',   # 警告
    'danger':        '#F06A6A',   # 错误/危险
...
配色逻辑：**中性石墨黑**（`bg_base`/`bg_elevated`/`bg_card` 三档表面）承载内容，**低饱和蓝** `accent` 作为唯一强调色，红/绿/黄保留为语义状态色。无渐变、无霓虹光晕，文字按主/次/辅助/禁用四档区分层级。
...
所有控件为**纯色扁平**样式，4~6px 小圆角，细边框，无阴影无渐变。
...
| `windowActive` | 标题栏/外框激活状态（true/false） |
| `windowMaximized` | 最大化状态（true/false） |

### 3.5 全状态覆盖

每个可交互控件均实现：**rest / hover / pressed / focus / disabled**。键盘焦点（`:focus`）与悬停（`:hover`）完全独立，焦点边框用 `focus` token（亮蓝），不随鼠标悬停显示。勾选框/单选框指示器统一为圆形（未选空心圆、已选绿色实心圆）。
...
└─ windowFrame QWidget            # 活动/非活动外框 (1px语义边框, 最大化隐藏)
   ...
   │   ├─ App图标 + 窗口标题+版本  #   （居左）
   │   ├─ 最小化 / 最大化还原 / 关闭  # （居右, 46x36, 关闭=Windows危险色）
   ├─ centralRoot (纯色底)
   │   ├─ QTabWidget              # 6个功能页
   │   │   ├─ 设备信息 / 刷入面具 / 刷Recovery / 文件管理
   │   │   ├─ 高级功能(含实用工具) / 投屏控制   (均包在 QScrollArea 内)
   │   ├─ 底部 QWidget
   │   │   ├─ 命令输出 QPlainTextEdit (5色日志标签 + 管理员指示器)
   │   │   └─ 二维码卡片 (2x2网格, 含"大图"弹窗)
   │   └─ QStatusBar
   │       ├─ 状态 QLabel
...
- **外框**：`#windowFrame` 1px 语义边框（活动=`window_border_active` 灰，失活=`window_border_inactive`）；最大化时边距置 0 并隐藏边框，让内容铺满工作区（不覆盖任务栏）
- **窗口控制按钮**：`CaptionButton` 自绘符号（46x36），状态由 QSS 驱动；关闭按钮 hover/pressed 使用危险色；最大化/还原按钮符号与工具提示随状态自动切换；每个按钮有 accessible name
- **缩放**：8 个不可见边缘/角落热区（6px 边 / 14px 角）委托 `QWindow.startSystemResize()`，不拦截中央区域鼠标事件
- **失活态**：`windowActive` 动态属性驱动标题栏/外框/标题文字变暗，保持可读性

### 4.1 静态头栏（AuroraHeader）

- 纯色石墨表面（`bg_elevated`）+ 底部 2px 低饱和蓝强调线
- **零动画、零 CPU 开销**，仅一次绘制
